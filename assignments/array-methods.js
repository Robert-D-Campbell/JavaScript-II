...
runners.map(runner => {
  firstNamesAllCaps.push(
    `${runner.first_name} ${runner.last_name}`.toUpperCase()
  );
});
...
const result = runners.filter(runners => runners.shirt_size === "L");
runnersLargeSizeShirt.push(result);
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = 0;
const totalPrice = runners.reduce(function(counter, runner) {
...
// Problem 1
// you would like to add all runners to your Email List. Return an Array using Map that displays only email addresses from the Runners.
let emailList = [];
runners.map(runner => {
...
// Problem 2
// You would like to send a thank you email to all runners who donated $200 or more. Return an Array that contains the Runners who donated $200 or more.
let topDonators = [];
const result2 = runners.filter(runners => runners.donation >= 200);
topDonators.push(result2);
